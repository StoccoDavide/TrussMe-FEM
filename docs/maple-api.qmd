...
Set the module options: verbose mode `VerboseMode`, warning mode `WarningMode`, time limit `TimeLimit`, id code length `IdLength`, node color `NodeColor`, support color `SupportColor`, element color `ElementColor`, shell (2+ nodes elements) color `ShellColor`, force color `ForceColor`, moment color `MomentColor`, node token `NodeToken`, support token `SupportToken`.

- **Inputs**: None
- **Optional inputs**:
  - `ElementColor::{nothing, string} := NULL`: element color
  - `ForceColor::{nothing, string} := NULL`: force color
  - `IdLength::{nothing, positive} := NULL`: ID length
  - `MomentColor::{nothing, string} := NULL`: moment color
  - `NodeColor::{nothing, string} := NULL`: node color
  - `NodeToken::{nothing, string} := NULL`: node token
  - `ShellColor::{nothing, string} := NULL`: shell color (2+ nodes elements)
  - `SupportColor::{nothing, string} := NULL`: support color
  - `SupportToken::{nothing, string} := NULL`: support token
  - `TimeLimit::{nothing, nonnegative} := NULL`: time limit
  - `VerboseMode::{boolean, nothing} := NULL`: verbose mode
  - `WarningMode::{boolean, nothing} := NULL`: warning mode
- **Returns**: `NULL`
...
## Reference Frames, Points and Vector
...
Generate a reference frame matrix from three points or vectors `p_1`, `p_2` and vector `vec` orthogonal to XY-plane `vec`.
...
Generate a reference frame matrix from three points or vectors `p_1`, `p_2` and vector `vec` orthogonal to XZ-plane `vec`.
...
Solve the FEM structure `fem` and optionally use LAST LU decompostion `use_LAST` and veil the expressions `use_LEM` with signature mode `use_SIG` and label `label`. Factorization method `factorization` can be choosen between `LU`, fraction-free `FFLU`, `QR`, and Gauss-Jordan `GJ`. Time limit `time_limit` and maximum veiling cost `maxcost` can be specified.

- **Inputs**: `fem::FEM`
- **Optional inputs**: `factorization::string := "LU"`, `label::string := "V"`, `maxcost::nonnegint := 15`, `time_limit::positive := 1.0`, `use_LAST::boolean := false`, `use_LEM::boolean := true`, `use_SIG::boolean := true`
- **Returns**: `NULL`

Proto: `SolveFEM(fem, factorization = "LU", label = "V", maxcost = 15, time_limit = 1.0, use_LAST = false, use_LEM = true, use_SIG = true)`
